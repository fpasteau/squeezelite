...
static void process_dsco(u8_t *pkt, int len) {
	...
	struct DSCO_packet pkt_send;
	struct DSCO_packet *pkt_receive = (struct DSCO_packet *)pkt;

	memset(&pkt_send, 0, sizeof(pkt_send));
	memcpy(&pkt_send.opcode, "DSCO", 4);
	pkt_send.length = htonl(sizeof(pkt_send) - 8);
	pkt_send.reason = pkt_receive->reason;

	send_packet((u8_t *)&pkt_send, sizeof(pkt_send));
}

static void sendSETDLine() {
	struct SETD_linelen pkt_linelen;

	memset(&pkt_linelen, 0, sizeof(pkt_linelen));
	memcpy(&pkt_linelen.opcode, "SETD", 4);

	pkt_linelen.id = 0xfe; // id 0 is playername S:P:Squeezebox2
	pkt_linelen.size = htonl(2);
	pkt_linelen.linelen = interactive.linelen;

	LOG_DEBUG("set linelen: %d", interactive.linelen);

	send_packet((u8_t *)&pkt_linelen, sizeof(pkt_linelen));
}

void slimproto_ir(int format, int noBits, int irCode) {
...
void *interactive_thread(void * att){
	...
	fd_set write_fds;
	int maxfd = 0;
	interactive_str *inter = (interactive_str *) att;
	   while (!inter->finished && (inter->using_curses || inter->use_lcdd_menu || inter->using_lirc)) {
	      FD_ZERO(&read_fds);
	      FD_ZERO(&write_fds);
	      if (inter->using_curses)
		 FD_SET(0, &read_fds); 
	      if  (inter->use_lcdd_menu) {
		 FD_SET(inter->lcd_fd, &read_fds); 
		 maxfd = inter->lcd_fd;
	      }
	      if (inter->using_lirc) {
		 FD_SET(inter->lirc_fd, &read_fds); 
		 if (inter->lirc_fd > maxfd) 
		    maxfd = inter->lirc_fd;
	      }
	      _timeout.tv_sec = 5;
	      if(select(maxfd + 1, &read_fds, NULL, NULL, &_timeout) == -1) {
#ifndef __WIN32__
		 if (errno != EINTR)
		 {
		 ...
		 while ((key = getch()) != ERR) {
		    ...
		    if (ir == (unsigned long) 0x01) {
		       slimproto_stop();
		       	...
		       	if (ir != 0) slimproto_ir(1, 1, ir);
		    }
		 }
	      }
	      if (inter->using_lirc && FD_ISSET(inter->lirc_fd, &read_fds)) {
		 while((key = read_lirc()) != 0 ) { 
	      ...
	      if (inter->use_lcdd_menu && FD_ISSET(inter->lcd_fd, &read_fds)) {
		 while(read_lcd()); 
	      }
	}
return NULL;
...
void interactive_init(void){
	if ( interactive.using_curses || interactive.using_lirc || interactive.use_lcdd_menu )
	{
		sendSETDLine();
	}
